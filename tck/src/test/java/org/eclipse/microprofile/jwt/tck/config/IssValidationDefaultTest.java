 ...
 *
 * Validate that if there is a {@linkplain Names#REQUIRE_ISS} property missing, validation against
 * the {@linkplain Names#ISSUER} property is performed.
 */
public class IssValidationDefaultTest extends Arquillian {
    /**
    ...
    public static WebArchive createDeployment() throws Exception {
        ...
        // The mp.jwt.verify.requireiss is not specified in this test, should default to true
        //configProps.setProperty(Names.REQUIRE_ISS, "true");
        configProps.setProperty(Names.ISSUER, TCKConstants.TEST_ISSUER);
        StringWriter configSW = new StringWriter();
        configProps.store(configSW, "IssValidationDefaultTest microprofile-config.properties");
        StringAsset configAsset = new StringAsset(configSW.toString());

        WebArchive webArchive = ShrinkWrap
            .create(WebArchive.class, "IssValidationDefaultTest.war")
    ...
    @Test(groups = TEST_GROUP_CONFIG,
        description = "Validate that JWK with iss and no mp.jwt.verify.requireiss returns HTTP_OK")
    public void testDefaultValidation() throws Exception {
        Reporter.log("testDefaultValidation, expect HTTP_OK");

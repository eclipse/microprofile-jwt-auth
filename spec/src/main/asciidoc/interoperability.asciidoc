...
A JWT can contain both "roles" and "groups", or just "groups" or "roles". If a JWT only contains "roles", the set
of names returned by this method is the set from the array of role names. If a "groups" claim exists in the JWT,
the set of names returned by this method is the set of role names that have been applied to the array of group names.

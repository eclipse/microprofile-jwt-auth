...
1. The JWT must have an iss claim representing the token issuer that maps to an MP-JWT implementation
...
1. The JWT signer must have a public key that that maps to an MP-JWT implementation container runtime
...
caller as a JsonWebToken with `@RequestScoped` or `@Dependent` scoping:
...
with `@RequestScoped` or `@Dependent` scoping. The following example code fragment illustrates various
...
This requires `@Dependent` scoping due to the final nature of most claim standard claim
values.

#### Injection of Claim Values As JSON-P Types
An MP-JWT implementation is required to support injection of the currently
authenticated `JsonWebToken` claim values using the `javax.json.JsonValue`
subtypes as the injection site type as shown in this code fragment:
...
...
    ...
    @Inject
    @Claim(standard = Claims.jti)
    private JsonString providerJTI;
    @Inject
    @Claim(standard = Claims.iat)
    private JsonNumber providerIAT;
    @Inject
    @Claim("groups")
    private JsonArray providerGroups;
    @Inject
    @Claim("customObject")
    private JsonObject customObject;
----

using `@RequestScoped` or `@Dependent` scoping.
...
Any role names used in @RolesAllowed or equivalent security constraint metadata that match role names in

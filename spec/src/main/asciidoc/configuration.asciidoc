...
In such cases, when the confidentiality of the claims is critical, the claims can be encrypted instead of being signed,
...
If the encrypted content is an inner nested JWT then its signature has to be verified by configuring the verification key as described in the <<signed-jwt-tokens, Signed JWT tokens>> section.
...
This specification does not support the application endpoints accepting signed or encrypted or signed and encrypted tokens at the same time. Requiring the implementations to determine a token type at runtime (signed token will have 3 parts, encrypted token - 5 parts) will have a small performance cost and it is expected that a given MP JWT application will accept only signed or only encrypted or only signed and encrypted tokens which will be enforced depending on how `mp.jwt.decrypt.key.location` and either `mp.jwt.verify.publickey.location` or `mp.jwt.verify.publickey` properties are set.

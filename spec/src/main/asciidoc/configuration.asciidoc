...
than use the matching Public Key to verify JWT and trust the user information (claims) it
contains.

The goal if this chapter is to detail means of passing the Public Key from the JWT Issuer
...
Attempts to parse the Public Key material will proceed in the order specified above until
...
The format consists of a Base64 encoded value wrapped in a standard `-----BEGIN PUBLIC
KEY-----` header and footer.  The Base42 encoded data can be decoded and the resulting
...
MicroProfile JWT implementations must inspect supplied public key text for the `-----BEGIN
PUBLIC KEY-----` header and parse the key as PCKS#8 if the header if found.

Support for legacy PKCS#1 format is not required and should be considered vendor-specific.
PKCS#1 formatted keys can be identified by the use of `-----BEGIN RSA PUBLIC KEY-----`
header and footer.
...
The JWK maybe supplied in plain JSON or Base64 encoded JSON format.
...
In the absence of a matching `kid`, all keys in the set will be tried in the order listed.

The JWKS maybe supplied in plain JSON or Base64 encoded JSON format.
...
By convention, property names are always lowercase and begin with `mp.jwt.`

#### `mp.jwt.verify.publickey`

The `mp.jwt.verify.publickey` config property allows the Public Key material itself to be
...
return Public Key material in one of the supported formats.
...
If the `mp.jwt.verify.issuer` config property has not been set, any issuer is allowed.

NOTE: In most cases relying on the Public Key signature check alone is sufficient to establish trust.